...
private:
    int convert_nalu_to_ps(uint8_t *pInput, uint32_t iInputSize, uint8_t **pOutput, uint32_t *pOutputSize);
...
int H264or5Fragmenter::convert_nalu_to_ps(uint8_t *pInput, uint32_t iInputSize, uint8_t **pOutput, uint32_t *pOutputSize)
{
    // Loge("Enter, iInputSize:%d", iInputSize);
    pInput[0] = 0;
    pInput[1] = 0;
    pInput[2] = 0;
    pInput[3] = 1;
...
#define USE_PS_STREAM        
        ...
        if (fCurDataOffset == 1){
            ...
            convert_nalu_to_ps(fInputBuffer, frameSize, &mpOutput, &fNumValidDataBytes);
            if (mpOutput == NULL){
                fCurDataOffset = 1;
        ...
        fLastFragmentCompletedNALUnit = True;
        // Loge("OutputSize:%d, fCurDataOffset:%d", fNumValidDataBytes, fCurDataOffset);
        if (fNumValidDataBytes < fMaxSize){
            memmove(fTo, mpOutput, fNumValidDataBytes);
            ...
            fLastFragmentCompletedNALUnit = False;
            memmove(fTo, mpOutput + fCurDataOffset, fFrameSize);
